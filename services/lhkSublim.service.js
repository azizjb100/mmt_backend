...
const getAllHeaders = async (startDate, endDate) => {
  const sql = `
        ...
        LEFT JOIN tlhk_rtr_dtl ON lrd_lr_nomor = lr_nomor
        WHERE lr_tanggal BETWEEN ? AND ?
        GROUP BY lr_nomor, lr_tanggal, lr_gdg_kode, gdg_nama
...
const getDetailsByNomor = async (nomor) => {
  const sqlDetail = `
        SELECT 
            ...
            COALESCE(lrd_qty_mutasi, 0) AS Qty_Mutasi,
            (lrd_jumlah - COALESCE(lrd_qty_mutasi, 0)) AS Sisa_Belum_Mutasi,
...
const saveLhk = async (data) => {
  ...
  try {
    ...
    if (isNew) {
      ...
      await conn.query("DELETE FROM tlhk_rtr_dtl WHERE lrd_lr_nomor = ?", [
        nomorLhk,
      ]);
    }
