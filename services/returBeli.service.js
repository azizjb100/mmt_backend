        ...
        if (isUpdate) {
            ...
            await connection.query(
                `UPDATE tret_hdr_mmt SET 
                    ...
                    date_modified=? 
                WHERE ret_nomor=?`,
                [Gudang, Tanggal, SupplierKode || '', NoPenerimaan || '', Keterangan, activeUser, serverTime, Nomor]
            );
            ...
            await connection.query(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [Nomor, Gudang, Tanggal, SupplierKode || '', NoPenerimaan || '', Keterangan, activeUser, serverTime]
            );
        }
...
exports.getAllReturBeli = async (options = {}) => {
    try {
        ...
        const sql = `
            ...
            FROM tret_hdr_mmt h
            WHERE h.ret_nomor LIKE ? OR h.ret_sup_kode LIKE ? OR h.ret_memo LIKE ?
            ORDER BY h.date_create DESC
            LIMIT ? OFFSET ?;
        `;

        const [rows] = await pool.query(sql, [searchPattern, searchPattern, searchPattern, Number(limit), Number(offset)]);
        return rows;
    } catch (error) {
