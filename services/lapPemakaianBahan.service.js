...
const getFullProductionReport = async (startDate, endDate, mesin) => {
    try {
        ...
        Object.keys(groupedByShift).forEach(shiftKey => {
            ...
            const tintaShift = inkMap[`${firstRowShift.tgl}_${firstRowShift.shift}`] || {};
            ...
            Object.keys(groupedLO).forEach(loKey => {
                ...
                const processedRows = group.map(row => {
                    ...
                    const sisaBisaPakaiP = isGabungan ? 0 : (parseFloat(row.sisaBisaPakaiP) || 0);
                    const sisaBisaPakaiL = isGabungan ? 0 : (parseFloat(row.sisaBisaPakaiL) || 0);

                    const ambilLuas = ambilP * ambilL;
                    const sisaBisaPakaiLuas = sisaBisaPakaiP * sisaBisaPakaiL;
                    const sisaRongsokLuas = isGabungan ? 0 : (parseFloat(row.wasteMeter) || 0);
                    const aktualLuasPakai = isGabungan ? 0 : (ambilLuas - sisaBisaPakaiLuas - sisaRongsokLuas);

                    // =========================================================
                    // KALKULASI WASTE & LOST UNTUK SPK BIASA (BUKAN GABUNGAN/LO)
                    // =========================================================
                    let wasteM2 = 0;
                    let wastePersen = 0;
                    let lostM2 = 0;
                    ...
                    if (!isGabungan) {
                        wasteM2 = sisaRongsokLuas; // AD5 = AB5
                        wastePersen = orderLuas > 0 ? (wasteM2 / orderLuas) * 100 : 0; // AE5 = AD5 / P5
                        lostM2 = aktualLuasPakai - orderLuas - toleransiM2; // AF5 = AC5 - P5 - F5
                        lostPersen = orderLuas > 0 ? (lostM2 / orderLuas) * 100 : 0; // AG5 = AF5 / P5
                        totalWasteM2 = wasteM2 + lostM2; // AH5 = AD5 + AF5
                        totalWastePersen = orderLuas > 0 ? (totalWasteM2 / orderLuas) * 100 : 0; // AI5 = AH5 / P5
                    }
                    ...
                    return {
                        ...
                        orderLuas: parseFloat(orderLuas.toFixed(1)),
                ...
                if (isGabungan) {
                    ...
                    finalReport.push({
                        ...
                        hasilPRoll: first.hasilPRoll,
                        hasilQty: sumHasilQty,
                        hasilLuas: parseFloat(sumHasilLuas.toFixed(1)),
                        ambilP: ambilP,
                        ambilL: ambilL,
                        ambilLuas: parseFloat(ambilLuas.toFixed(1)),
                        ...
                        aktualLuasPakai: parseFloat(aktualLuasPakai.toFixed(1)),
                        wasteM2: parseFloat(wasteM2.toFixed(1)),
                        wastePersen: sumOrderLuas === 0 ? 0 : parseFloat(((wasteM2 / sumOrderLuas) * 100).toFixed(1)),
                        lostM2: parseFloat(lostM2.toFixed(1)),
                        lostPersen: sumOrderLuas === 0 ? 0 : parseFloat(((lostM2 / sumOrderLuas) * 100).toFixed(1)),
                        totalWasteM2: parseFloat(totalWasteM2.toFixed(1)),
                        totalWastePersen: sumOrderLuas === 0 ? 0 : parseFloat(((totalWasteM2 / sumOrderLuas) * 100).toFixed(1)),
