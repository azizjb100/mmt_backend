...
const getReport = async (startDate, endDate) => {
  ...
  const ssql = `
    SELECT 
      ...
      IFNULL(brg_panjang, 0) * IFNULL(brg_lebar, 0) AS m2,
      
      IFNULL(stok_awal.total_stok, 0) AS stok_awal_q,
      IFNULL(stok_awal.total_stok, 0) * (IFNULL(brg_panjang, 0) * IFNULL(brg_lebar, 0)) AS stok_awal_m,
      
      IFNULL(trans.rec, 0) + IF(IFNULL(trans.mut, 0) > 0, trans.mut, 0) + IF(IFNULL(trans.kor, 0) > 0, trans.kor, 0) - IFNULL(trans.retsup, 0) AS terima_q,
      (IFNULL(trans.rec, 0) + IF(IFNULL(trans.mut, 0) > 0, trans.mut, 0) + IF(IFNULL(trans.kor, 0) > 0, trans.kor, 0) - IFNULL(trans.retsup, 0)) * (IFNULL(brg_panjang, 0) * IFNULL(brg_lebar, 0)) AS terima_m,
      
      IFNULL(trans.prod, 0) + IF(IFNULL(trans.mut, 0) < 0, ABS(trans.mut), 0) + IF(IFNULL(trans.kor, 0) < 0, ABS(trans.kor), 0) AS keluar_q,
      (IFNULL(trans.prod, 0) + IF(IFNULL(trans.mut, 0) < 0, ABS(trans.mut), 0) + IF(IFNULL(trans.kor, 0) < 0, ABS(trans.kor), 0)) * (IFNULL(brg_panjang, 0) * IFNULL(brg_lebar, 0)) AS keluar_m,
      
      IFNULL(trans.retprod, 0) AS retur_q,
      (IFNULL(trans.retprod, 0)) * (IFNULL(brg_panjang, 0) * IFNULL(brg_lebar, 0)) AS retur_m,
      
      IFNULL(stok_akhir.total_stok, 0) AS stok_akhir_q,
      IFNULL(stok_akhir.total_stok, 0) * (IFNULL(brg_panjang, 0) * IFNULL(brg_lebar, 0)) AS stok_akhir_m
