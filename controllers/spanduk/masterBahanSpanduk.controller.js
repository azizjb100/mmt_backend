const bahanService = require("../../services/spanduk/masterBahanSpanduk.service");
...
exports.getMasterBahan = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Gagal memuat daftar barang.", error: error.message });
    }
};
...
exports.getBahanDetail = async (req, res) => {
    try {
        ...
        if (!String(kode || "").trim()) {
            return res.status(400).json({ message: "Kode barang wajib diisi." });
        }
        const data = await bahanService.getBahanByKode(kode);
        return res.status(200).json({ data });
    } catch (error) {
        return res.status(404).json({ message: "Kode Barang tidak ditemukan.", error: error.message });
    }
};
...
exports.saveMasterBahan = async (req, res) => {
    try {
        ...
        const userLogin = String(
            req.user?.kdUser || req.user?.KDUSER || req.user?.username || 
            req.user?.user_nama || req.user?.user || "SYSTEM"
        );
        const result = await bahanService.saveBahan(req.body, userLogin);
        return res.status(200).json({ message: "Simpan berhasil", kode: result.kode });
    } catch (error) {
        return res.status(500).json({ message: "Gagal simpan master barang.", error: error.message });
    }
};
...
exports.lookupKategori = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Gagal memuat lookup kategori.", error: error.message });
    }
};
...
exports.lookupGudang = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Gagal memuat lookup gudang.", error: error.message });
    }
};
...
exports.lookupSupplier = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Gagal memuat lookup supplier.", error: error.message });
    }
};
...
exports.lookupJenis = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Gagal memuat lookup jenis.", error: error.message });
    }
};
...
exports.lookupDivisi = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Gagal memuat lookup divisi.", error: error.message });
    }
};